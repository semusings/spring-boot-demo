...
public interface PaymentCommand {
  ...
  @PostMapping("/{orderId}")
  PaymentResponse createPayment(@PathVariable String orderId, @RequestBody PaymentRequest request);
  ...
  record PaymentResponse(String paymentId, String orderId, String status, String customerId) {
    ...
    public static PaymentResponse of(Payment order) {
      return new PaymentResponse(order.getPaymentId().S(), order.getOrderId().S(),
          order.getStatus().name(), order.getCustomerId().S());
    }
  }
}
