...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    public static class JwtTokenUtil {
        ...
        public static boolean validateToken(String token) {
            return (!isEmpty(getUsernameFromToken(token))
                    && !isEmpty(getUserEmailFromToken(token))
        ...
        private static <T> T getClaimFromToken(String token, String claimKey, Class<?> type) {
            ...
            throw new InvalidTokenAuthenticationException(
                    "Key "
                            + claimKey
                            + " does not match with expected return type "
                            + type.getSimpleName());
        }

        private static <T> T getClaimObjectFromToken(
                String token, Function<JWTClaimsSet, T> claimsResolver) {
